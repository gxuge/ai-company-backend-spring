...
### T1：请求模型与规范化
- 输入：前端三个结构化场景选项。
- 执行动作：新增公共选项 DTO，在生图请求 DTO 中接入并限制合法 key 与描述长度。
- 输出：生图接口可接收新字段，提示词优化接口保持原契约。
- 验收标准：旧请求仍可通过；非法 key 不进入 Prompt。已完成。
- 证据类型：DTO 单测与编译结果。

### T2：Prompt 变量与模板
- 输入：DTO 中的 key/description。
- 执行动作：扩展生图变量构建器，更新生图数据库 Prompt 模板变量。
- 输出：生图模型能读取时间、天气、气氛描述；优化模板保持原样。
- 验收标准：生图渲染变量包含六个场景选项字段，优化模板不发生变化。已完成。
- 证据类型：变量单测、SQL 静态检查。
...
- 风险：前端暂时同时把描述拼入 `siteSetting`，生图模板可能重复读取。
- 缓解：结构化字段作为主来源；后续前端可将 `siteSetting` 收敛为自由输入。
- 回退：回滚 Java DTO/变量改动，并恢复 `airag_prompts` 中生图模板的上一版本内容。
...
# 20260830-simplify-preset-tag-model

## 元信息
- 任务名称：精简角色标签与生成预设标签模型
- 分级：H2
- 负责人：Codex
- 开始时间：2026-08-30
- 关联：角色创建、角色/故事预设生成、AI 预设管理

## 目标与非目标
- 目标：删除 `ts_role_tag`、`ts_tag_relation`、`ts_user_role_tag`、`ts_user_preference_tag`、`ts_preset_tag` 五张旧表及对应代码。
- 目标：保留角色/故事 preset 生成接口和 Prompt 模板，改为仅使用 `ts_preset` 名称、描述及用户输入。
- 目标：移除角色创建页标签选择和管理端预设标签关联入口。
- 非目标：不删除 `ts_preset`、`ts_tag_type`、`ts_tag`，不改变 preset 接口地址。

## 任务分解
- [x] T1：清理后端旧 Controller、Service、Mapper、Entity、VO 及标签关联逻辑。
- [x] T2：将角色/故事 preset 生成改为预设描述驱动，并精简生成快照。
- [x] T3：移除 Native 角色标签区域和 Vue3 预设标签管理入口。
- [x] T4：新增删表迁移、更新 API/变更文档并完成定向验证。

## 验证矩阵
| 验证项 | 方法 | 期望 |
|---|---|---|
| 后端编译 | Maven 目标模块编译 | 不存在旧类型或 Mapper 引用 |
| 前端检查 | Native 定向转译、Vue3 类型检查 | 角色创建与预设管理无废弃引用 |
| 静态扫描 | `rg` 扫描旧表和旧 Java 类型 | 仅历史迁移、基线 SQL 和文档历史可命中 |
| 数据库迁移 | 本地 MySQL 执行并查询表 | 五张旧表均不存在 |

## 风险与回退
- 风险：五张旧表的数据会被永久删除；其中 `ts_preset_tag` 当前有历史关联数据。
- 风险：预设生成结果不再受标签粒度控制，质量主要依赖 `ts_preset.description`。
- 回退：代码可回退到改造前版本；表结构可参考历史迁移重建，旧关联数据需从数据库备份恢复。

## 未完成项
- 未启动完整应用实际调用角色/故事 preset 接口。
- Vue3 全量构建受现有压缩环境变量缺失影响，已改用目标 SFC 解析和 TypeScript 转译验证。

## 验证记录
- Maven Reactor 8 个模块编译成功。
- Web `bun run check:types` 通过。
- Native 目标角色创建页和 API 索引通过 Babel Expo 转译；全量类型检查的既有错误未命中本次文件。
- Vue3 目标 SFC 解析和 API TypeScript 转译通过。
- 本地 MySQL 已删除五张旧表，两份 preset Prompt 已更新，31 条启用预设描述均非空。

# 20260831-content-tagging-foundation
...
- 负责人：Codex
- 开始时间：2026-08-31
- 关联：角色/故事生成、作品审核快照、固定标签词典

## 目标与非目标
- 目标：将旧生成素材标签改造成角色/故事固定内容标签词典。
- 目标：生成接口可返回候选标签，角色/故事保存后按内容版本校验并写入内容标签。
- 目标：候选标签缺失时复用作品审核快照执行独立异步 AI 打标，并记录可重试任务状态。
- 非目标：不修改行为埋点、不新增用户偏好表、不实现推荐排序。
- 非目标：不改动 `ts_voice_tag`、`ts_voice_profile_tag` 语音标签体系。

## 任务分解
- [x] T1：新增数据库迁移，重建固定标签词典并创建内容标签、打标任务表。
- [x] T2：实现候选标签 DTO、固定词典校验、版本覆盖存储和任务状态管理。
- [x] T3：扩展角色/故事生成及保存结构，接入生成候选标签落库。
- [x] T4：复用审核快照增加异步缺失标签兜底。
- [x] T5：补充单元测试、API 文档并执行定向编译验证。

## 验证矩阵
| 验证项 | 方法 | 期望 |
|---|---|---|
| 标签校验 | JUnit 定向测试 | 非固定标签、低分标签被过滤，每类最多 3 个 |
| 版本保护 | JUnit 定向测试 | 旧版本任务不能覆盖最新内容标签 |
| 生成兼容 | Maven 编译 | 原生成字段不变，新增 `tags` 为可选字段 |
| 异步兜底 | 事件监听测试/代码检查 | 事务提交后进入独立线程池，不阻塞作品保存 |
| 数据结构 | SQL 静态检查 | 词典、内容标签、任务表及回滚说明完整 |

## 风险与回退
- 风险：旧 `ts_tag_type/ts_tag` 数据将被固定内容标签完全替换。
- 风险：Prompt 未返回 `tags` 时会增加一次独立 AI 打标调用。
- 风险：异步任务可能因模型或模板故障失败，失败状态保留并允许重试。
- 回退：恢复迁移前标签表备份，删除 `ts_content_tag`、`ts_content_tag_task`，回退对应 Java 改动。

## 未完成项
- 本机 Docker 服务未运行，未对本地 MySQL 实际执行 `V3.9.1_58__create_content_tagging_foundation.sql`。
- 角色/故事现有生成 Prompt 尚未增加 `tags` 字段；代码已兼容可选字段，当前缺失时由独立打标 Prompt 兜底。

## 验证记录
- Maven Reactor 8 个模块编译成功。
- 目标模块测试源码编译成功。
- `TsContentTagServiceImplTest` 2 条测试成功、0 失败，覆盖固定词典、分数阈值、每类数量和版本保护。
- `git diff --check` 未发现空白错误；语音标签和行为埋点未纳入本次迁移或新服务。

# 20260831-behavior-tag-snapshot
...
- 负责人：Codex
- 开始时间：2026-08-31
- 关联：业务行为 Kafka、ClickHouse 明细、角色/故事内容标签

## 目标与非目标
- 目标：复用现有行为链路，由服务端为角色/故事事件补充内容版本和标签快照。
- 目标：将事件结构升级为 v3，并扩展 ClickHouse 明细字段。
- 目标：取消收藏成功时新增可信 `unfavorite` 事件。
- 非目标：不新增 `dislike`、故事进度或故事完成事件。
- 非目标：不新增用户偏好表，不实现偏好计算和推荐排序。

## 任务分解
- [x] T1：实现角色/故事行为标签快照批量补全。
- [x] T2：升级 Kafka 消息和 ClickHouse 明细结构。
- [x] T3：接入 `unfavorite` 可信业务事件。
- [x] T4：补充测试、API/配置文档并执行定向验证。

## 验证矩阵
| 验证项 | 方法 | 期望 |
|---|---|---|
| 标签补全 | JUnit 定向测试 | 使用当前内容版本，仅写入该版本标签 |
| 批量采集 | JUnit 定向测试 | 整批补全后发布，客户端不能提交标签 |
| 取消收藏 | 代码检查/测试 | 仅真实取消成功时产生 `unfavorite` |
| ClickHouse 映射 | 消费者测试/SQL 检查 | 内容版本、标签 ID 和分数完整写入 |
| 模块回归 | Maven 编译与定向测试 | 构建成功，既有事件兼容 |

## 风险与回退
- 风险：行为上报会增加角色、故事及标签的批量查询。
- 风险：已有 ClickHouse 数据卷不会自动执行新增初始化脚本。
- 回退：回退 v3 Java 字段和 SQL 映射；ClickHouse 新增列可保留为空，不影响 v2 数据读取。

## 未完成项
- 未连接真实 Kafka 和 ClickHouse 执行端到端冒烟。
- 已有 ClickHouse 数据卷需要在部署环境手动执行 `02_user_behavior_tag_snapshot.sql`。

## 验证记录
- Maven Reactor 8 个模块测试源码编译成功。
- 5 组定向测试共 18 条通过，0 失败、0 错误。
- 覆盖当前版本标签过滤、ClickHouse 字段映射、客户端拒绝伪造 `unfavorite`、
  真实取消与重复取消的事件边界。

# 20260831-content-tag-display
...
- 负责人：Codex
- 开始时间：2026-08-31
- 关联：内容标签词典、角色/故事查询接口、Native 展示页

## 目标与非目标
- 目标：角色与故事的本人列表、详情、保存响应及公开列表、详情返回当前版本标签。
- 目标：前端在角色详情、故事浏览卡片和故事详情展示标签名称。
- 目标：对外仅暴露标签标识、类型和名称，不暴露分数、来源、模型版本等内部字段。
- 非目标：不修改标签生产、审核、行为埋点、用户偏好与推荐逻辑。
- 非目标：不新增或修改数据库表。

## 任务分解
- [ ] T1：新增标签展示 VO 和当前版本批量查询能力。
- [ ] T2：接入角色/故事本人及公开查询响应。
- [ ] T3：接入 Native 角色详情、故事卡片和故事详情展示。
- [ ] T4：更新 API、ADR、变更文档并完成定向验证。

## 验证矩阵
| 验证项 | 方法 | 期望 |
|---|---|---|
| 版本过滤 | JUnit 定向测试 | 只返回请求内容当前版本标签 |
| 接口编译 | Maven 目标模块测试编译 | 新增 VO 和服务调用无编译错误 |
| 前端类型 | TypeScript/Babel 定向检查 | 新增 `tags` 字段和展示组件可转译 |
| 静态检查 | `git diff --check` | 无空白错误与意外编码变化 |

## 风险与回退
- 风险：列表接口增加内容标签、标签词典和类型词典批量查询。
- 风险：尚未完成打标的内容返回空标签数组，前端不展示标签区。
- 回退：移除响应 `tags/contentVersion` 字段和前端标签区域，无数据库回滚。

# 20260902-preset-template-tag-compatibility
...
- 负责人：Codex
- 开始时间：2026-09-02
- 关联：角色/故事预设生成、固定标签词典、内容审核异步打标

## 目标与非目标
- 目标：保持 `role_core_fill_preset:v2`、`story_core_fill_preset:v2` 旧模板内容不变。
- 目标：从启用标签词典中识别预设和用户输入包含的标签，并补齐旧模板变量。
- 目标：正式内容标签继续由现有审核快照异步 AI 打标，不把生成前提示标签直接落库。
- 非目标：不新增预设标签关系表，不修改生成接口 DTO/VO，不修改 Prompt 数据库内容。

## 任务分解
- [x] T1：新增预设标签变量适配器。
- [x] T2：接入角色和故事预设生成变量构建。
- [x] T3：补充标签识别、禁用标签和缺失变量测试。
- [x] T4：完成定向测试、模块编译和静态检查。

## 验证矩阵
| 验证项 | 方法 | 期望 |
|---|---|---|
| 标签识别 | JUnit 定向测试 | 仅识别当前 scope 下启用且文本明确包含的标签 |
| 旧变量补齐 | JUnit 定向测试 | 角色和故事旧模板变量均有值或 `null` |
| 生成链路 | Maven 模块编译 | 两个生成服务依赖注入及调用可编译 |
| 静态检查 | `git diff --check` | 无空白错误与意外模板变更 |

## 风险与回退
- 风险：预设生成每次增加一次小型启用标签词典查询。
- 风险：仅做明确词面匹配，同义词不会被生成前提示标签推断。
- 回退：移除适配器注入和 `putAll` 调用即可恢复原变量构建，无数据库回滚。

## 验证记录
- Maven Reactor 8 个模块编译及测试源码编译成功。
- 使用 JUnit Platform 直接执行 `TsPresetPromptVariableAdapterTest`，2 条测试通过、0 失败。
- `git diff --check` 未发现空白错误。
- 未修改 `role_core_fill_preset:v2`、`story_core_fill_preset:v2` 数据库模板。

# 20260903-user-follow-resource-like
...
## 目标与非目标
- 目标：提供用户关注、取消关注、状态、关注列表和粉丝列表。
- 目标：提供角色/故事点赞、取消点赞、状态和点赞列表。
- 目标：写操作幂等，禁止关注自己，仅允许点赞在线公开资源。
- 非目标：不修改前端，不改变现有收藏接口，不接入推荐行为埋点。

## 任务分解
- [x] T1：新增用户关注和资源点赞数据库迁移。
- [x] T2：新增 Entity、DTO、PO、VO 与 Mapper。
- [x] T3：实现 Service、Controller 与权限边界。
- [x] T4：补充定向测试、API 文档和变更记录。

## 验证矩阵
| 验证项 | 方法 | 期望 |
|---|---|---|
| 关注规则 | JUnit 定向测试 | 禁止关注自己；重复关注/取消幂等 |
| 点赞规则 | JUnit 定向测试 | 仅公开资源可点赞；重复点赞/取消幂等 |
| 接口编译 | Maven 目标模块测试编译 | 新增分层代码与 Mapper XML 可编译 |
| 静态检查 | XML 解析与 `git diff --check` | Mapper XML 合法，无空白错误 |

## 风险与回退
- 风险：用户列表直接读取 `sys_user`，仅返回正常且未删除用户。
- 风险：点赞总数采用关系表实时聚合，数据量增长后可增加统计表异步汇总。
- 回退：下线新增 Controller，并删除两张关系表；现有收藏和业务数据不受影响。

## 验证记录
- Maven Reactor 8 个模块测试源码编译成功。
- 用户关注与资源点赞定向测试共 7 条通过。
- 两个 Mapper XML 解析及方法映射检查通过。
- 新增接口不修改现有收藏、公开角色和公开故事响应结构。
