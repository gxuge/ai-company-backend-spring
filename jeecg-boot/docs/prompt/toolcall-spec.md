...
### 4.1 预设 + 标签取值（故事预设链路）
- 预设读取：
  - 从 `ts_preset` 查询 `target_type='story'` 且 `enabled=1`。
  - 按 `sort_order asc, id asc` 排序后随机选取一条可用预设。
- 关系读取：
  - 从 `ts_preset_tag` 按 `preset_id` 查询绑定关系。
  - 按 `sort_order asc, id asc` 保持稳定顺序。
- 标签读取：
  - 收集关系中的 `tag_id` 去重后，从 `ts_tag` 批量查询标签实体。
  - 以 `type_id` 分组，按关系顺序聚合 `name`，并做去重。
  - 多值使用英文逗号加空格拼接（`, `）。
- 变量注入：
  - 将分组后的标签值与业务输入一起注入模板变量 Map。
