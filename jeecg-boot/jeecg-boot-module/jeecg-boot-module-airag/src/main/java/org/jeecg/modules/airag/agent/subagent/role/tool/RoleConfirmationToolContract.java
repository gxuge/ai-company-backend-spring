 ...
 * 角色确认展示工具契约。
 *
 * <p>只维护前端确认区域的动态文案，不承载角色数据和流程跳转。</p>
 *
 * @author codex
 * @date 2026/7/17
 */
public final class RoleConfirmationToolContract {
    ...
    private static final String TOOL_DESCRIPTION =
            "角色四项核心设定整理完成后，用于向用户展示确认问题和两个候选文案。"
    ...
    private static final String INPUT_SCHEMA = """
            {
              ...
              "properties": {
                "question": {
                  "type": "string",
                  "maxLength": 12,
                  "description": "询问用户是否喜欢当前角色版本。文案由你自行决定，语气亲切自然，可以带符号或表情，不超过12个字。"
                },
                "confirmLabel": {
                  "type": "string",
                  "maxLength": 12,
    ...
    private RoleConfirmationToolContract() {
    }
    ...
    public static ToolSpecification buildSpecification() {
        JsonObjectSchema schema = JsonObjectSchema.builder()
                .addStringProperty(QUESTION, "询问用户是否喜欢当前角色版本。" + COPY_REQUIREMENT)
    ...
    private static String requireCopy(Map<String, Object> arguments, String fieldName) {
        ...
        if (!oConvertUtils.isNotEmpty(value)) {
            throw new AgentErrorException(
                    AgentErrorCode.TOOL_ROLE_CONFIRMATION_REQUIRED_FIELD_MISSING,
                    Map.of("field", fieldName)
            );
        }
        if (value.codePointCount(0, value.length()) > MAX_COPY_LENGTH) {
            throw new AgentErrorException(
                    AgentErrorCode.TOOL_ROLE_CONFIRMATION_FIELD_TOO_LONG,
