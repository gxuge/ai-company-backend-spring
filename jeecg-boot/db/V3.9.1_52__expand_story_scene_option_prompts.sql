...
UPDATE airag_prompts
SET
    description = '故事场景图片提示词优化模板 v1，支持时间、天气和气氛选项描述',
    content = 'TEMPLATE_BEGIN::story_scene_image_prompt_optimize::v1
SECTION::meta
...
你的任务：把用户提供的已有故事场景描述，结合时间、天气和气氛选项描述，整理成可直接用于文生图的中文视觉提示词。

要求：
1. 只优化，不改原意，不乱加地点、时代、人物或剧情设定。
2. 保留场景核心信息、地点、时间天气、空间结构、关键陈设、环境氛围、光线、色彩、镜头、材质和视觉风格。
3. time_description、weather_description 和 mood_description 是用户选择的视觉约束，必须优先体现。
4. 语言要简练、具体、稳定，避免空话、重复和抽象形容。
5. 视觉提示词尽量控制在120到180字，必要时不要超过220字。
6. 负面提示词尽量控制在30到60字，不要超过80字。
7. 默认输出中文，只保留一个主风格，不混用互相冲突的风格词。
8. 画面应突出环境叙事、空间层次和背景可读性，避免无关人物近景、界面元素、文字、水印和logo。
9. 如果用户原文已经完整，只做轻度润色和压缩；如果原文过短，只补充最必要的构图信息，不要过度扩写。
10. 不要输出解释、分析、Markdown、列表、代码块或多余前后缀。
11. 必须且只能通过tool call输出，且只能调用一次工具：submit_story_scene_image_prompt_optimize。
12. 不要生成违规内容，不要复刻受限人物或版权场景。
13. 后端白名单只提取visual_prompt和negative_prompt，禁止新增其他业务字段。
...
SECTION::user_prompt_template
{
  ...
  "time": {
    "key": "{{time_key}}",
    "description": "{{time_description}}",
    "description_cn": "用户选择的时间条件"
  },
  "weather": {
    "key": "{{weather_key}}",
    "description": "{{weather_description}}",
    "description_cn": "用户选择的天气条件"
  },
  "mood": {
    "key": "{{mood_key}}",
    "description": "{{mood_description}}",
    "description_cn": "用户选择的环境气氛条件"
  }
}
...
SECTION::tool_schema
{
  ...
  "strict": true,
  "description": "提交故事场景图片提示词优化结果。必须仅通过tool call返回结构化JSON参数，不得输出解释、Markdown或其他前后缀文本；只能返回visual_prompt和negative_prompt。",
  "parameters": {
    "type": "object",
    "additionalProperties": false,
    "properties": {
...
TEMPLATE_END::story_scene_image_prompt_optimize::v1',
    update_by = 'admin',
    update_time = NOW()
WHERE prompt_key = 'story_scene_image_prompt_optimize'
  AND version = 'v1'
  AND del_flag = 0;
