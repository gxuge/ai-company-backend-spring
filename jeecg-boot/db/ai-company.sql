...
CREATE TABLE `ts_story_info` (
  ...
  `site_setting` TEXT COMMENT '故事设定/场景设定',
  ...
  `plot_outline` TEXT COMMENT '剧情大纲',
...
CREATE TABLE ts_user_voice_profile (
    ...
    KEY idx_voice_profile_id (voice_profile_id),
    CONSTRAINT fk_user_voice_profile_user
        FOREIGN KEY (user_id) REFERENCES sys_user (id),
...
INSERT INTO `ts_story_info` (`story_code`, `title`, `story_mode`, `site_setting`, `scene_id`, `scene_name_snapshot`, `status`, `is_ai_story_setting`, `is_ai_character`, `is_ai_outline`, `plot_outline`, `created_by`, `created_name`, `updated_by`, `updated_name`, `is_deleted`)
VALUES
